        ...
        >
            ...
            <div
                ...
                style={{
                    ...
                    border: hovered ? `1px solid ${s.accent}30` : "1px solid rgba(255,255,255,0.07)",
                }}
            >
                {/* Icon bubble */}
                <div
                    ...
                    style={{
                        background: hovered ? `${s.accent}20` : "rgba(255,255,255,0.04)",
                        border: `1px solid ${hovered ? s.accent + "40" : "rgba(255,255,255,0.08)"}`,
                ...
                <div className="flex-1 min-w-0">
                    <p className="text-xs uppercase tracking-widest font-medium mb-0.5" style={{ color: "rgba(255,255,255,0.3)" }}>{s.label}</p>
                    <p className="text-sm font-semibold truncate" style={{ color: "rgba(255,255,255,0.8)" }}>{s.value}</p>
                    <p className="text-xs mt-0.5" style={{ color: "rgba(255,255,255,0.25)" }}>{s.desc}</p>
                </div>
                {/* Arrow */}
                <span
                    className="flex-shrink-0 text-sm transition-all duration-300"
                    style={{ color: hovered ? s.accent : "rgba(255,255,255,0.15)", transform: hovered ? "translateX(3px)" : "translateX(0)" }}
                >→</span>
            </div>
        </motion.a>
    )
    ...
    return (
      ...
      `}</style>
            ...
            <div className="fixed inset-0 pointer-events-none overflow-hidden" style={{ zIndex: 0 }}>
                <div style={{ position: "absolute", top: "5%", right: "-10%", width: 600, height: 600, background: "radial-gradient(circle, rgba(59,130,246,0.09) 0%, transparent 70%)", borderRadius: "50%", filter: "blur(60px)" }} />
                <div style={{ position: "absolute", bottom: "10%", left: "-10%", width: 500, height: 500, background: "radial-gradient(circle, rgba(99,102,241,0.08) 0%, transparent 70%)", borderRadius: "50%", filter: "blur(50px)" }} />
            </div>
            <svg className="fixed inset-0 w-full h-full pointer-events-none" style={{ opacity: 0.025, zIndex: 0 }} xmlns="http://www.w3.org/2000/svg">
                <defs><pattern id="ct-grid" width="48" height="48" patternUnits="userSpaceOnUse"><path d="M 48 0 L 0 0 0 48" fill="none" stroke="rgba(148,180,255,1)" strokeWidth="0.5" /></pattern></defs>
            ...
            <div className="relative" style={{ zIndex: 2 }}>
                ...
                <section className="pt-40 pb-20 px-6 text-center" ref={headerRef}>
                    <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="inline-flex items-center gap-3 mb-8">
                        ...
                        <span className="text-xs uppercase tracking-widest font-medium" style={{ color: "rgba(148,180,255,0.7)" }}>Estamos aquí</span>
                        <div className="h-px w-10" style={{ background: "rgba(148,180,255,0.5)" }} />
                    </motion.div>
                ...
                <section className="pb-32 px-6">
                    <div className="max-w-6xl mx-auto grid lg:grid-cols-5 gap-10">
                        ...
                        <div className="lg:col-span-2 flex flex-col gap-8" ref={socialsRef}>
                            ...
                            >
                                <p className="text-xs uppercase tracking-widest font-medium mb-5" style={{ color: "rgba(148,180,255,0.5)" }}>
                                    Información
                                </p>
                                <div className="flex flex-col gap-4">
                                    {info.map((item, i) => (
                                        <div key={i} className="flex items-center gap-3">
                                            <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.15)", color: "#818cf8" }}>
                                                {item.icon}
                                            </div>
                                            <div>
                                                <p className="text-xs" style={{ color: "rgba(255,255,255,0.25)" }}>{item.label}</p>
                                                <p className="text-sm font-medium" style={{ color: "rgba(255,255,255,0.7)" }}>{item.value}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                                ...
                                {/* Availability badge */}
                                <div className="mt-6 flex items-center gap-2.5 pt-5" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                                    <motion.span
